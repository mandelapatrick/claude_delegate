...
import { getOnboardingStatusHandler, openOnboardingHandler, } from "./tools/onboarding.js";
import { searchBrainHandler, addBrainEntryHandler, } from "./tools/manage-brain.js";
import { listMeetingsHandler } from "./tools/list-meetings.js";
import { getAgentStatusHandler, getMeetingBriefHandler, } from "./tools/sessions.js";
...
server.tool("get_onboarding_status", "Check the current onboarding status of the user's AI delegate setup", {}, async () => ({
    content: [{ type: "text", text: await getOnboardingStatusHandler() }],
}));
server.tool("open_onboarding", "Open the onboarding wizard in the user's browser to set up their AI delegate", {}, async () => ({
    content: [{ type: "text", text: await openOnboardingHandler() }],
}));
server.tool("search_brain", "Search the user's second brain (PARA knowledge base) for relevant context", {
...
server.tool("add_brain_entry", "Add a new entry to the user's second brain PARA structure", {
...
server.tool("list_meetings", "List upcoming meetings from the user's Google Calendar", {
    days: z
        .number()
        .optional()
        .default(1)
...
server.tool("get_agent_status", "Check the status of active AI delegate sessions in meetings", {}, async () => ({
    content: [{ type: "text", text: await getAgentStatusHandler() }],
}));
server.tool("get_meeting_brief", "Get post-meeting briefs with summaries and action items", {
    session_id: z
        .string()
        .optional()
        .describe("Specific session ID to get brief for. If omitted, returns recent briefs."),
}, async (args) => ({
