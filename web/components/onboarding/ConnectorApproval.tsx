...
const CONNECTORS = [
  {
    ...
    description: "Access your meeting schedule and event details",
    icon: (
...
export default function ConnectorApproval({ onComplete }: ConnectorApprovalProps) {
  ...
  const toggle = (id: string) => {
    if (id === "calendar") return; // Calendar is always connected via Google OAuth
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-zinc-500 text-xs">
          Your delegate uses these to understand your work and respond
          intelligently.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {CONNECTORS.map((c) => (
          <button
            ...
            onClick={() => {
              toggle(c.id);
              // After toggling, pass current state
              const updated = { ...connectors, [c.id]: c.id === "calendar" ? true : !connectors[c.id] };
              onComplete(updated);
            }}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {c.autoConnected && (
                  <span className="text-[10px] px-1.5 py-0.5 bg-green-900/50 text-green-400 rounded-full">
                    Auto-connected
                  </span>
                )}
              </div>
